...
Two rules, both enforced on every file under src/main (Kotlin and Java):

  1. No DJI import or fully-qualified reference (`dji.`, `com.cySdkyc.`).
  2. No reference to a class that is declared only in a flavor source set. Same-package
     ...
     flavor-declared class, with comments and string literals stripped first.
...
def main() -> int:
    ...
    print(
        f"OK: {len(source_files(main_root))} main source files reference no DJI and no "
