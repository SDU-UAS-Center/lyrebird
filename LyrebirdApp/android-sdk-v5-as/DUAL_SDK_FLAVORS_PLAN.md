...
Also configure and run Spotless, Detekt/Lint, and release-variant build checks for the new source roots. Inspect both resolved runtime classpaths and merged manifests/APKs. From the repository root, run `uv run --locked pytest GroundStation/tests -q` and the applicable Python quality gates when shared client contracts are touched.
