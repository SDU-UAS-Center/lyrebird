...
| 9 | The existing `FlightDeckActivity` remains the canonical class name and manifest entry, but its DJI V5 implementation now lives under `lyrebird-app/src/v5/java`. Current and demo variants register that shell through their existing Kotlin compile tasks; `src/main` no longer owns the platform entry point. No V4 flavor or V4 adapter work has started. | Add the V4 source registration and neutral platform shell only when beginning flavor implementation; then perform explicit lifecycle/device qualification. |
