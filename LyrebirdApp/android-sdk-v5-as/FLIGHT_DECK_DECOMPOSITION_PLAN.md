...
| 6 | Target selection, WHIP endpoint construction, and V5 WebRTC streamer/listener construction are extracted and tested; native livestream lifecycle and full activity-independent publisher ownership remain. | Move native stream and Wi-Fi-lock lifecycle behind a coordinator. |
| 7 | Detection port and neutral source/active/model/threshold telemetry projection are extracted and tested; V5 onboard/local providers remain activity-wired. | Move provider lifecycle and overlay/frame ownership behind a coordinator. |
