...
| 8 | Runtime startup now stops after a blocked/non-serving session, activity teardown releases the lease last, and `ProcessNetworkRuntime` owns HTTP/telemetry/discovery sockets with weak, replaceable activity bindings. SDK bootstrap is lease-gated. | Move fleet/MAVLink/WebRTC ownership into the same process runtime and verify activity recreation, SDK initialization, and lease ordering on-device. |
