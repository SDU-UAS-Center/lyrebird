...
| 5 | HTTP command boundary now exposes neutral media, detection, and LRF ports; DJI/ViewModel/UXSDK types are removed from `LyrebirdCommandHost`. Existing route behavior remains in place, and payload/flight implementation ownership has not moved yet. | Extract the shared flight command policy and move motion/mission sinks without changing Safety, RC override, or response semantics. |
| 6-7 | Not started: streaming coordination and detection coordination. | Focused tests and the corresponding bench gates below. |
