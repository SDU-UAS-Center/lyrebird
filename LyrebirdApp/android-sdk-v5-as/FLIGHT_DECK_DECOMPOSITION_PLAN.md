...
| 6 | Target selection, WHIP endpoint construction, V5 WebRTC streamer construction, V5 native RTMP/RTSP/Agora/GB28181 lifecycle, and process-scoped publisher trigger/Wi-Fi-lock ownership are extracted behind streaming helpers and `ProcessStreamingRuntimeRegistry`. Native-mode start callbacks still pass through the activity's V5 coordinator. | Device qualification of each streaming mode and activity recreation. |
...
| 8 | Runtime startup now stops after a blocked/non-serving session, activity teardown releases the lease last, `ProcessNetworkRuntime` owns HTTP/telemetry/discovery sockets and their multicast lock with weak, replaceable activity bindings, `FleetMeshSessionRegistry` owns multicast link/roster state, `ProcessMavlinkRuntimeRegistry` owns the MAVLink UDP endpoint plus FTP server, `ProcessStreamingRuntimeRegistry` owns the WebRTC publisher, WHIP target state, and low-latency Wi-Fi lock, `ProcessObstacleRuntimeRegistry` owns the obstacle singleton callbacks, `ProcessCaptureExecutorRegistry` owns the capture queue, and `ProcessSettingsBackupRuntimeRegistry` owns the debounced settings listener/writer across activity recreation. MAVLink, streaming, obstacle, capture, and settings bindings are weak or neutral and replaceable. SDK bootstrap is lease-gated. | Verify activity recreation, SDK initialization, lease ordering, obstacle braking, settings persistence, discovery multicast, and long-lived streaming on-device; native-mode callbacks still pass through the activity adapter. |
...
skips. The activity is 4,112 lines, down from 7,665. The documentation build passed.
