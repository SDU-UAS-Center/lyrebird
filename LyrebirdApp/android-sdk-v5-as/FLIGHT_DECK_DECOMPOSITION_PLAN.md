...
| 8 | Runtime startup now stops after a blocked/non-serving session, activity teardown releases the lease last, `ProcessNetworkRuntime` owns HTTP/telemetry/discovery sockets with weak, replaceable activity bindings, `FleetMeshSessionRegistry` owns multicast link/roster state, `ProcessMavlinkRuntimeRegistry` owns the MAVLink UDP endpoint plus FTP server, and `ProcessStreamingRuntimeRegistry` owns the WebRTC publisher, WHIP target state, and low-latency Wi-Fi lock across activity recreation. MAVLink and streaming command, motion, mission, snapshot, video, parameter, logging, peer, media, metrics, and native-mode bindings are weak and replaceable. SDK bootstrap is lease-gated. | Move capture/FTP executor policy and obstacle ownership into the same process runtime; verify activity recreation, SDK initialization, and lease ordering on-device. |
...
skips. The activity is 4,153 lines, down from 7,665. The documentation build passed.
