...
| 7 | Detection port and neutral source/active/model/threshold telemetry projection are extracted and tested; V5 AutoSensing lifecycle and SDK target conversion now live in `V5AutoSensingProvider`. Local TFLite provider, overlay, and frame ownership remain activity-wired. | Move local provider lifecycle and overlay/frame ownership behind a coordinator. |
