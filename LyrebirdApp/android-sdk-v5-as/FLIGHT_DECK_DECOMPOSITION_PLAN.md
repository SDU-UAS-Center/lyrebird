...
| 4 | `V5AircraftTelemetrySource` owns key reads, battery subscriptions, and high-frequency telemetry observers. Core `AircraftReadings` and shared projections feed TCP, MAVLink, and fleet output. | Generation/freshness handling and a canonical state store are not implemented. Samples are still obtained at each existing emission; video metadata retains its separate V5 telemetry provider. Connection/flight-state callbacks remain in the activity. |
| 5-7 | Not started: command/mission extraction, streaming coordination, and detection coordination. | Focused tests and the corresponding bench gates below. |
