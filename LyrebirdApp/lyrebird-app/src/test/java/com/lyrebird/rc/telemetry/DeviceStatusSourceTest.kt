...
class DeviceStatusSourceTest {
    ...
    /**
     * The phone half of the frame is served by the process, so a reading has to reach whoever
     * installed the hook — that is what keeps the RC's battery and position moving in the
     * published telemetry while no screen is attached.
     */
    @Test
    fun everyPhoneSideChangePublishesASnapshot() {
        val source = DeviceStatusSource(DeviceServicesContext())
        val published = mutableListOf<DeviceStatusSnapshot>()
        source.onChanged = { published += it }

        source.notifyChanged()

        assertEquals(1, published.size)
        assertEquals("no fix yet, no battery manager: the honest defaults", -1, published.single().battery)
        assertEquals(-100, published.single().wifiRssi)
    }
}