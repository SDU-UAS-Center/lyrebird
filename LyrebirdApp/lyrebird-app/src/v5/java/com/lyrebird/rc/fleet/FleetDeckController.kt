...
internal class FleetDeckController(
    ...
    ownPosition: () -> Pair<Double, Double>?,
) {
    companion object {
    ...
    private val mapCamera = FleetMapCamera(mapWidget, ownPosition)
    ...
    fun start() {
        ...
        mapOverlay.setCompact(!mapExpanded)
        // The map follows this aircraft whether or not the fleet mesh is on: with no peers it is
        // simply an aircraft on a map, which is what it was before there was a fleet at all.
        mapCamera.install()
        if (!isEnabled) {
     ...
     *
     * The map's own size is part of the camera policy: the compact map cannot show a fleet at the
     * zoom the expanded one can, so both the icons and the framing change with it.
     */
    fun setMapExpanded(expanded: Boolean) {
        mapExpanded = expanded
        if (expanded) stripView?.visibility = View.GONE
        mapOverlay.setCompact(!expanded)
        mapCamera.setExpanded(expanded)
        // Straight back on collapse, rather than at the next beacon tick: the fleet the operator
        // was looking at a second ago is still there.
        if (!expanded) refresh()
    }
    ...
    private fun refresh() {
        ...
        // The camera frames what the overlay drew, so it is handed the same snapshot rather than
        // reading the roster again on its own clock. Places include the last known one for a peer
        // whose fix has dropped out, because the overlay draws those too.
        mapCamera.setPeers(
            view.peers.mapNotNull { it.place }.map { DJILatLng(it.latitudeDeg, it.longitudeDeg) },
        )
    }
    ...
    fun showFleetDialog() {
        val view = currentView()
        val count = view.memberCount
        AlertDialog
            .Builder(activity)
            .setTitle("Fleet ($count aircraft)")
    ...
    private fun fleetPageText(view: FleetView): String =
        buildString {
            if (view.isEmpty) {
                ...
                append("AIRCRAFT\n")
                view.own?.let { own -> append(ownLine(own)).append('\n') }
    ...
    private fun ownLine(own: FleetOwnView): String =
        if (own.hasPosition) {
            "• ${own.displayName} · this aircraft"
        } else {
            "• ${own.displayName} · this aircraft · no fix of its own yet"
        }
    ...
    private fun peerLine(peer: FleetPeerView): String {
        ...
        val age =
            peer.solutionAgeMs
                ?.let { " · last fix ${it / MILLIS_PER_SECOND}s ago" }
                .orEmpty()
        ...
        val closing =
            peer.solution
                ...
                .orEmpty()
        return "• ${peer.displayName} (MAV ${beacon.systemId}) · $state$range$age$battery$closing"
    }
