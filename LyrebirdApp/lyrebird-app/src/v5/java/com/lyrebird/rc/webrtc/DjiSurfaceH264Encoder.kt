 ...
 * [encode] only exists to satisfy the [VideoEncoder] interface, to translate a WebRTC keyframe
 ...
 * [PeriodicKeyframeEncoder]'s frame-type trick -- and to hand [InputTimestampQueue] the capture
 * timestamps that make encoded output deliverable, see the pairing rule below. Encoded frames are
 * handed to WebRTC by a dedicated delivery thread paced at the target fps, independent of the
 * engine's [encode] call rate: flight 4 showed the engine calling [encode] at only 3-6/s during
 * flight (captured-frame drops under CPU contention), which starved the previous encode()-driven
 * drain and caused the observed queue-overflow cascade.
 *
 * The frames WebRTC hands [encode] are also the frame pairing libwebrtc insists on. A Java
 * encoder's output is only accepted when its `captureTimeNs` matches one of the timestamps
 * recorded for an input frame still queued in `VideoEncoderWrapper`; MediaCodec's own
 * presentation timestamps come from DJI's surface frames and never match those, so every frame
 * has to be re-stamped from [inputTimestamps] before delivery or the RTP sender never sees a
 * single packet -- the whole session dies with MediaMTX's "deadline exceeded while waiting
 * tracks" while MediaCodec, the drain and the callbacks all look perfectly healthy.
 */
internal class DjiSurfaceH264Encoder(
    ...
    private companion object {
        ...
        // Several seconds of input timestamps at any supported frame rate. The queue only has to
        // absorb the difference between capture cadence and codec output cadence; the cap exists
        // to bound memory (and the age of the oldest entry) if the codec stops producing frames.
        const val TIMESTAMP_QUEUE_CAPACITY = 120
    }
    ...
    // Filled by the pipeline thread in [encode], drained by the delivery thread: the only
    // timestamps libwebrtc's VideoEncoderWrapper will accept an encoded frame back with.
    private val inputTimestamps = InputTimestampQueue(TIMESTAMP_QUEUE_CAPACITY)
    ...
    private fun deliverNextFrame() {
        ...
        // Peek before consuming: a queued frame may only leave together with the input timestamp
        // that makes it deliverable, and until one is pending the frame simply waits for a later
        // tick. Consuming a timestamp no frame is using would desynchronize the two queues and
        // stall the stream instead.
        val pending = pendingOutputs.peek() ?: return
        val captureTimeNs = inputTimestamps.nextForDelivery() ?: return
        pendingOutputs.poll()
        val encodedImage =
            EncodedImage
                .builder()
                ...
                // Not the codec's own presentation time: see the class doc and
                // [InputTimestampQueue] -- libwebrtc drops any mismatch before RTP framing.
        ...
        if (callbackCount == 1L || callbackCount % DIAGNOSTIC_LOG_INTERVAL == 0L) {
            Log.i(
                ...
                "Delivered encoded output: callback=$callbackCount size=${pending.size} " +
                    ...
                    "captureTimeNs=$captureTimeNs timestampsEvicted=${inputTimestamps.evictions} " +
                    "stampStarvations=${inputTimestamps.starvations}",
            )
        }
    }
    ...
    override fun encode(
        ...
        // The pixels are unused (see the class doc); the timestamp is not -- it is the only stamp
        // libwebrtc will accept the matching encoded output back with.
        frame?.let { inputTimestamps.record(it.timestampNs) }
    ...
    override fun release(): VideoCodecStatus {
        ...
        Log.i(
            ...
            "Released: encodeCalls=${encodeCalls.get()} outputBuffers=${outputBuffers.get()} " +
                ...
                "timestampsEvicted=${inputTimestamps.evictions} " +
                "stampStarvations=${inputTimestamps.starvations}",
        )
        return VideoCodecStatus.OK
    }
