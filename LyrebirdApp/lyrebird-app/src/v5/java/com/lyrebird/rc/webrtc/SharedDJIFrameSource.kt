...
class SharedDJIFrameSource(
    ...
    fun setEdgeDetectionFrameListener(listener: EdgeDetectionFrameListener?) {
        ...
        edgeDetectionActive.set(listener != null)
        if (FrameSourcePolicy.wantsCapture(observers.size, edgeDetectionActive.get())) {
            ensureFrameListenerAttached("edge detection or a viewer wants frames")
        } else if (isCapturing.compareAndSet(true, false)) {
    ...
    fun stopClient(clientId: String) {
        ...
        if (!FrameSourcePolicy.wantsCapture(observers.size, edgeDetectionActive.get()) && isCapturing.compareAndSet(true, false)) {
...
private data class DjiFrameRecipients(
    val observers: FrameRecipients<CapturerObserver>,
    val metadataListeners: FrameRecipients<DJIV5VideoCapturer.FrameMetadataListener>,
) {
    val hasObservers: Boolean = !observers.isEmpty
    val hasMetadataListeners: Boolean = !metadataListeners.isEmpty

    /**
     * Hand the frame to every observer.
     *
     * The reference arithmetic is [FrameRecipients]'s, tested there: this is the SDK-typed call
     * site, and the reason a second viewer used to be a class of crash rather than a feature.
     */
    fun deliver(videoFrame: VideoFrame) {
        observers.deliver(
            videoFrame,
            retain = { frame -> frame.retain() },
            hand = { observer, frame -> observer.onFrameCaptured(frame) },
        )
    }
    ...
    companion object {
        fun capture(
            ...
            DjiFrameRecipients(
                observers = FrameRecipients.of(observers.values),
                metadataListeners = FrameRecipients.of(metadataListeners.values),
            )
    }
}
