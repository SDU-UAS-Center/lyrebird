    ...
    fun initMobileSDK(appContext: Context) {
        ...
        SDKManager.getInstance().init(appContext, object : SDKManagerCallback {
            override fun onRegisterSuccess() {
                ...
                // The SDK is usable from here and not before: keys cannot be created ahead of
                // registration. This is therefore where the process attaches what reads the
                // aircraft — telemetry, its frame, the serial, the video publisher's settings —
                // rather than waiting for a screen to be opened (see ProcessAppRuntime).
                ProcessAppRuntime.onSdkRegistered(appContext)
            }
