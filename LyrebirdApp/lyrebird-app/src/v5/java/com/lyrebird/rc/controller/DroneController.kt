...
object DroneController {
    ...
    /**
     * The aircraft left the ground, seen from its own telemetry rather than from a command.
     *
     * This exists because Lyrebird is not the only thing that can fly this aircraft. The UXSDK's
     * own take-off control sits on the Flight Deck, and the RC's buttons are always there: both
     * command the flight controller directly, and neither goes through [startTakeOff]. Without
     * this the status stayed IDLE through a take-off nobody told Lyrebird about, which reads
     * wrongly in the status view and, worse, answers [isTakeoffStillClimbing] with "no" — so a
     * mission started in that state would fly its first leg while still climbing.
     *
     * Only an idle aircraft can be "taking off": anything the app is already doing (a landing, a
     * return to home, an active override) outranks a state inferred from telemetry.
     */
    internal fun onTelemetryTakeoff() {
        if (droneStatus == DroneStatus.IDLE) setDroneStatus(DroneStatus.TAKING_OFF)
    }

    /**
     * The aircraft is on the ground again, seen from its own telemetry.
     *
     * Clears only the states a take-off can leave behind; a landing or a return to home that the
     * app itself commanded keeps its own status until its own completion path clears it.
     */
    internal fun onTelemetryLanded() {
        if (droneStatus == DroneStatus.TAKING_OFF || droneStatus == DroneStatus.HOVERING) {
            setDroneStatus(DroneStatus.IDLE)
        }
    }
    ...
    private fun seedFlightLimits() {
        goHomeHeightKey.get(
            ...
            { error -> Log.w("DroneController", "Could not read RTH altitude: ${djiErrorReason(error)}") },
        )
        maxFlightHeightKey.get(
            { value -> value?.let { cachedMaxFlightHeight = it } },
            { error -> Log.w("DroneController", "Could not read max flight height: ${djiErrorReason(error)}") },
        )
        maxFlightDistanceKey.get(
            { value -> value?.let { cachedMaxFlightDistance = it } },
            { error -> Log.w("DroneController", "Could not read max flight distance: ${djiErrorReason(error)}") },
        )
        distanceLimitEnabledKey.get(
            { value -> value?.let { cachedDistanceLimitEnabled = it } },
            { error -> Log.w("DroneController", "Could not read distance limit: ${djiErrorReason(error)}") },
        )
    }

    /**
     * A DJI error rendered for a human.
     *
     * DJI leaves [IDJIError.description] blank or literally "null" for most core errors, so the
     * message the user and the log ended up with was the error's identifier
     * ("REQUEST_HANDLER_NOT_FOUND"), which names neither the cause nor the cure. The one code worth
     * naming is the one this class keeps provoking: it is what an unreachable flight controller
     * answers with, so the aircraft is standing by rather than misconfigured.
     */
    private fun djiErrorReason(error: IDJIError): String {
        val description = error.description().orEmpty().trim()
        if (description.isNotEmpty() && !description.equals("null", ignoreCase = true)) return description
        val code = error.errorCode()
        return when (code) {
            "REQUEST_HANDLER_NOT_FOUND" ->
                "the aircraft did not answer (flight controller offline or in standby)"
            else -> "DJI error code $code"
        }
    }

    /**
     * Whether the flight controller is answering right now, from KeyManager's cache only.
     *
     * False covers both "no aircraft" and DJI's low-power standby, where the product link stays up
     * while the flight-controller link drops. The flight limits live in the flight controller, not
     * in the phone, so a write sent in that state cannot land. Internal so the process-scoped
     * aircraft-settings surface can refuse an RC setting write for the same reason the limit
     * writes refuse one, instead of reporting a success the aircraft never received.
     */
    internal fun isFlightControllerConnected(): Boolean =
        runCatching { FlightControllerKey.KeyConnection.create().get(false) }.getOrDefault(false)

    /**
     * Refuse a flight-limit write the aircraft cannot receive, and say why. True when refused.
     *
     * Sending it anyway is what produced "RTH altitude change refused: DJI error code
     * REQUEST_HANDLER_NOT_FOUND" next to a silent no-op over HTTP, neither of which names the
     * actual situation: the aircraft is asleep in standby and will accept the setting once it
     * wakes. Refusing loudly leaves room for an operator to do the useful thing.
     */
    private fun refuseStandbyFlightLimitWrite(settingLabel: String): Boolean {
        if (isFlightControllerConnected()) return false
        Log.i(
            "DroneController",
            "$settingLabel not changed: flight controller not connected (aircraft in standby)",
        )
        ToastUtils.showToast("Aircraft in standby: $settingLabel not changed")
        return true
    }
    ...
    /**
     * Re-read the flight limits from the aircraft.
     *
     * Called on every connection change: the seed above runs once per process — and the first
     * settings read can easily happen while the aircraft is still asleep, so it seeds nothing —
     * while the listeners push `-1` the moment the aircraft stops answering. Without this, an
     * aircraft that slept and woke again kept reporting "not reported" for RTH altitude, max
     * height and max distance over HTTP and MAVLink until the app was restarted.
     */
    fun refreshFlightLimits() {
        setupFlightLimitListeners()
        seedFlightLimits()
    }
    ...
    fun setRTHAltitude(
        ...
        if (refuseStandbyFlightLimitWrite("RTH altitude")) {
        ...
        fun attemptSet(attempt: Int) {
            goHomeHeightKey.set(
                ...
                { error ->
                    val reason = djiErrorReason(error)
    ...
    fun setMaxFlightHeight(
        ...
        if (refuseStandbyFlightLimitWrite("max flight height")) {
            onResult?.invoke(false)
            return
        }
        maxFlightHeightKey.set(
            height,
            {
                cachedMaxFlightHeight = height
                ToastUtils.showToast("Max flight height set to $height m")
                onResult?.invoke(true)
            },
            { error ->
                Log.w("DroneController", "Max flight height not changed: ${djiErrorReason(error)}")
                onResult?.invoke(false)
            },
        )
    }
    ...
    fun setMaxFlightDistance(
        distance: Int,
        onResult: ((Boolean) -> Unit)? = null,
    ) {
        if (refuseStandbyFlightLimitWrite("max flight distance")) {
            onResult?.invoke(false)
            return
        }
        maxFlightDistanceKey.set(
            distance,
            {
                cachedMaxFlightDistance = distance
                ToastUtils.showToast("Max flight distance set to $distance m")
                onResult?.invoke(true)
            },
            { error ->
                Log.w("DroneController", "Max flight distance not changed: ${djiErrorReason(error)}")
                onResult?.invoke(false)
            },
        )
    }
    ...
    fun setDistanceLimitEnabled(
        enabled: Boolean,
        onResult: ((Boolean) -> Unit)? = null,
    ) {
        if (refuseStandbyFlightLimitWrite("distance limit")) {
            onResult?.invoke(false)
            return
        }
        distanceLimitEnabledKey.set(
            enabled,
            {
                cachedDistanceLimitEnabled = enabled
                ToastUtils.showToast("Distance limit ${if (enabled) "enabled" else "disabled"}")
                onResult?.invoke(true)
            },
            { error ->
                Log.w("DroneController", "Distance limit not changed: ${djiErrorReason(error)}")
                onResult?.invoke(false)
            },
        )
    }
