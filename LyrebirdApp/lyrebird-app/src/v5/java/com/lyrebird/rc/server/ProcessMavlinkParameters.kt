...
internal object ProcessMavlinkParameters {
    ...
    fun apply(
        ...
        when (name) {
            // The three limit writes report the aircraft's own answer rather than a fabricated
            // success: while it is in standby they are refused without being sent, and a
            // ground station that is told ACCEPTED for a setting the aircraft never received has
            // no way to tell the difference.
            PARAM_MAX_HEIGHT ->
                awaitParameterWrite { done -> DroneController.setMaxFlightHeight(value.toInt(), done) }

            PARAM_MAX_DISTANCE ->
                awaitParameterWrite { done -> DroneController.setMaxFlightDistance(value.toInt(), done) }

            PARAM_DISTANCE_LIMIT ->
                awaitParameterWrite { done -> DroneController.setDistanceLimitEnabled(value >= 0.5f, done) }
