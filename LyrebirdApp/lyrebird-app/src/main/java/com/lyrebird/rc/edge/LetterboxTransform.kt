...
class LetterboxTransform(
    ...
    fun containsModelPixel(
        modelX: Int,
        modelY: Int,
    ): Boolean = modelX in padX until padX + resizedWidth && modelY in padY until padY + resizedHeight

    fun sourceX(modelX: Int): Int = ((modelX - padX) / scale).toInt().coerceIn(0, frameWidth - 1)

    fun sourceY(modelY: Int): Int = ((modelY - padY) / scale).toInt().coerceIn(0, frameHeight - 1)
    ...
    private fun toNormalizedFrameX(inputX: Float): Float =
        ((inputX - padX) / scale).coerceIn(0f, frameWidth.toFloat()) / frameWidth.toFloat()

    private fun toNormalizedFrameY(inputY: Float): Float =
        ((inputY - padY) / scale).coerceIn(0f, frameHeight.toFloat()) / frameHeight.toFloat()

    private fun toInputCoordinate(value: Float): Float = if (value <= NORMALIZED_COORDINATE_MAX) value * inputWidth else value
