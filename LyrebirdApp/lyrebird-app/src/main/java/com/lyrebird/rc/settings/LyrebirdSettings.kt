...
/** Preferences that travel with the aircraft, swapped by [DroneSettingsProfiles] on serial change. */
        ...
        /** What the app stores in place of a serial it could not read (see [applyAutomaticDroneName]). */
        internal const val UNKNOWN_SERIAL = "UNKNOWN"

        /**
         * The name derived from an aircraft's serial, for a device nobody has renamed.
         *
         * The tail of the serial, because the head is shared by a production batch; sanitised,
         * because the name becomes a URL path, a mDNS service name and a ROS namespace.
         */
        fun deriveDroneName(serial: String): String {
            val suffix =
                serial
                    .trim()
                    .takeLast(8)
        ...
        /** The model's own single-class default, used when no labels file is chosen or readable. */
        internal val DEFAULT_EDGE_LABELS = listOf("person")
    ...
    /**
     * The name this aircraft publishes under: the operator's, or one derived from its serial.
     *
     * One rule for every consumer, because the name is an address, not a label: discovery
     * answers with it, the WHIP publisher publishes under it, telemetry reports it, and the
     * ground station builds its WHEP URL from it. Two consumers deriving it differently is how a
     * dashboard came to subscribe to a path nothing published to (see the discovery reply).
     *
     * The serial-derived form was the screen's ([PREF_DRONE_NAME_USER_SET] decides whether the
     * operator's name stands); it lives here so the process can answer it too, for a device whose
     * screen has not been opened or whose activity is already gone.
     */
    fun droneName(serial: String): String =
        sharedPreferences
            .getString(PREF_DRONE_NAME, "")
            ?.trim()
            .takeUnless { it.isNullOrEmpty() }
            ?: deriveDroneName(serial)

    /**
     * Write the serial-derived name, for an aircraft the operator has not named.
     *
     * Called wherever a serial appears, which is the only moment the name can be derived — and
     * the reason this exists: a device that boots with its aircraft switched off cannot read the
     * serial at startup, resolved `UNKNOWN` into `lb_unknown`, stored that, and kept answering
     * with it (in its own UI, in discovery, telemetry and the dashboard) for the rest of the
     * session, because nothing re-derived the name when the aircraft finally appeared.
     *
     * @return the name now in effect, or null when it did not change: the operator set one,
     *   the serial is still unknown, or the derived name is already stored.
     */
    fun applyAutomaticDroneName(serial: String): String? {
        if (sharedPreferences.getBoolean(PREF_DRONE_NAME_USER_SET, false)) return null
        if (serial.isBlank() || serial == UNKNOWN_SERIAL) return null
        val derived = deriveDroneName(serial)
        if (sharedPreferences.getString(PREF_DRONE_NAME, "") == derived) return null
        sharedPreferences.edit().putString(PREF_DRONE_NAME, derived).apply()
        return derived
    }
    ...
    /**
     * Persist the detection source.
     *
     * The legacy edge-detection flag is kept in step rather than cleared: [getDetectionSource]
     * still consults it for configurations that predate [PREF_DETECTION_SOURCE], so leaving it
     * stale would make an older reader disagree with this one about the same aircraft.
     */
    fun setDetectionSource(source: DetectionSource) {
        sharedPreferences
            .edit()
            .putString(PREF_DETECTION_SOURCE, source.prefValue)
            .putBoolean(PREF_EDGE_DETECTION_ENABLED, source == DetectionSource.YOLO_ON_PHONE)
            .apply()
    }

    /** Persist whether detections run at all, with the legacy flag in step as above. */
    fun setDetectionsEnabled(enabled: Boolean) {
        sharedPreferences
            ...
            .putBoolean(
                ...
                enabled && getDetectionSource() == DetectionSource.YOLO_ON_PHONE,
            ).apply()
    }
    ...
    fun setDjiSurfaceH264EncoderEnabled(enabled: Boolean) {
        sharedPreferences.edit().putBoolean(WebRTCPeerFactory.PREF_USE_DJI_SURFACE_H264_ENCODER, enabled).apply()
    }
    ...
    internal fun getEdgeModelUri(): Uri? = sharedPreferences.getString(PREF_EDGE_MODEL_URI, null)?.let(Uri::parse)

    /**
     * The labels file chosen alongside the edge model, read through [resolver].
     *
     * Falls back to the model's own single-class default rather than an empty list: an unreadable
     * or unset labels file must not make every detection nameless. Reading lives here rather than
     * in a screen so the process-wide detector resolves labels the same way with no screen open.
     */
    internal fun getEdgeLabels(resolver: ContentResolver): List<String> {
        val labelsUri =
            sharedPreferences
                .getString(PREF_EDGE_LABELS_URI, null)
                ?.let(Uri::parse)
                ?: return DEFAULT_EDGE_LABELS
        val labels =
            runCatching {
                resolver
        ...
        return labels.ifEmpty { DEFAULT_EDGE_LABELS }
    }
}
