    ...
    fun summary(): String =
        when {
            ...
            !isServing -> "Not serving: ${failures.joinToString(", ").ifEmpty { "no servers started" }}"
                ...
                // orEmpty, not the raw nullable: a healthy session's summary ended in the word
                // "null" on every device, which is exactly the line an operator reads to find out
                // whether the aircraft is serving.
                "Serving ${serving.joinToString(" and ")}${down.orEmpty()}"
            }
        }
}
...
internal class LyrebirdSession(
    ...
    private companion object {
        const val TAG = "LyrebirdSession"
    }

    /**
     * Takes the lease, binds the servers, and advertises what came up.
     *
     * Safe to call again, and a *partially* bound session is retried rather than accepted: each
     * server's start is idempotent, the lease is already ours, and a port that was taken at boot
     * is often free later. The screen calls this on every attach, which is the only retry a
     * ground station cannot make for itself — a half-bound session used to stay half bound for
     * the life of the process.
     *
     * @return what the session is actually doing; see [LyrebirdSessionStatus].
     */
    fun start(): LyrebirdSessionStatus {
        if (status.httpPort != null && status.telemetryPort != null) return status
        ...
        val wasPartial = status.isServing
        val httpUp = status.httpPort != null || http.start()
        val telemetryUp = status.telemetryPort != null || telemetry.start()
        ...
        if (wasPartial) Log.i(TAG, "Network session retried: ${status.summary()}")
        return status
    }
