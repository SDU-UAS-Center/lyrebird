...
class TelemetryCoordinator {
    ...
    // Streaming Config
    // The mode's outbound name, so the telemetry stream, GET /config and LYREBIRD_CONFIG all
    // spell the same video path the same way.
    @Volatile var streamingMode: String = StreamingMode.WEBRTC.wireName
