...
data class FleetBeacon(
    ...
    companion object {
        ...
        /** Not a state message: a device withdrawing an identity it advertised earlier. */
        const val TYPE_RETIRE = "retire"
        ...
        /**
         * The datagram a device sends when it retires the identity it was advertising.
         *
         * A device's id changes under it exactly once per session that matters: the app starts,
         * has no aircraft serial yet, and advertises its installation id; the link comes up and the
         * serial takes over. Peers key the roster on the device id, so without this the same
         * aircraft stays in every peer's list twice for the whole silence timeout — once live under
         * the new id and once stale under the old one — and since both rows carry the same MAVLink
         * id and the same name, the conflict detector then raises a critical fleet conflict about a
         * device that is not in conflict with anything. The stale row is the thing to remove; the
         * new one is already on its way.
         */
        fun retireBytes(deviceId: String): ByteArray =
            JSONObject()
                .apply {
                    put(KEY_MAGIC, MAGIC)
                    put(KEY_VERSION, PROTOCOL_VERSION)
                    put(KEY_TYPE, TYPE_RETIRE)
                    put(KEY_DEVICE_ID, deviceId)
                }.toString()
                .toByteArray(Charsets.UTF_8)

        /** The id a retire datagram names, or null when this is not one we understand. */
        fun parseRetire(
            data: ByteArray,
            length: Int,
        ): String? =
            runCatching {
                ...
                if (obj.optString(KEY_MAGIC) != MAGIC) return null
                if (obj.optInt(KEY_VERSION) != PROTOCOL_VERSION) return null
                if (obj.optString(KEY_TYPE) != TYPE_RETIRE) return null
                obj.optString(KEY_DEVICE_ID).trim().ifEmpty { null }
            }.getOrNull()
    }
}
