...
internal class DeviceStatusSource(
    ...
    /**
     * A phone-side reading arrived: location fix, heading, pressure.
     *
     * The process installs this (see the telemetry runtime) so the published frame carries the
     * RC's battery and position whether or not a screen exists. Before, the only consumer was the
     * Flight Deck's cache rebuild, so the phone half of the telemetry froze the moment the screen
     * went away — while the aircraft half kept moving, because that projection is process-owned.
     */
    @Volatile internal var onChanged: ((DeviceStatusSnapshot) -> Unit)? = null
    ...
    /** One current read of everything phone-side, for whoever serves it. */
    ...
    /** Tell the owner (the process) that a phone-side value moved. May run on any thread. */
