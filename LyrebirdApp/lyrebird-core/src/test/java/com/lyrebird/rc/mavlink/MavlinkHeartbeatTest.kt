...
class MavlinkHeartbeatTest {
    private fun customMode(snapshot: MavlinkSnapshot): Long =
        ByteBuffer
            .wrap(MavlinkMessages.heartbeat(snapshot))
            .order(ByteOrder.LITTLE_ENDIAN)
            .int
            .toLong() and 0xFFFFFFFFL

    private fun baseMode(snapshot: MavlinkSnapshot): Int = ByteBuffer.wrap(MavlinkMessages.heartbeat(snapshot)).get(6).toInt() and 0xFF
