...
class MavlinkAddressingTest {
    ...
    @Test
    fun `a frame names the system that sent it`() {
        // The framer is system 200. This is how the endpoint recognises the datagrams that come
        // back from its own broadcast socket instead of filing itself as a ground station.
        val frame = framer.frame(MavlinkMsgId.COMMAND_LONG, commandLongPayload(targetSystem = 1))
        assertEquals(200, MavlinkInbound.senderSystemOf(frame, frame.size))
    }
    ...
    fun `a corrupt frame yields no addressing information`() {
        ...
        assertNull(MavlinkInbound.heartbeatAutopilot(frame, frame.size))
        assertNull(MavlinkInbound.senderSystemOf(frame, frame.size))
    }
