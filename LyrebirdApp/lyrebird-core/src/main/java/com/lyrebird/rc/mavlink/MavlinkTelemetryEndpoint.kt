 ...
 */
class MavlinkTelemetryEndpoint(
    ...
    private fun receiveLoop(bound: DatagramSocket) {
        ...
        while (running && !bound.isClosed) {
            ...
            if (packet.length > 0 && buffer[0] == MavlinkFramer.MAGIC_V2) {
                ...
                // This aircraft broadcasts its telemetry from the socket it also listens on, so
                // every frame it sends comes straight back - and only the heartbeat is caught by
                // the vehicle check above. A frame carrying this aircraft's own system id is
                // therefore not a ground station: registering it made the aircraft discover
                // itself and publish its whole video stream to its own address.
                if (MavlinkInbound.senderSystemOf(buffer, packet.length) == config.systemId) continue
