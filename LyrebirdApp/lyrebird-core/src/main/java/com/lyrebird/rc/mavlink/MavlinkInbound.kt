...
data class MavlinkMissionCount(
    val count: Int,
    val missionType: Int,
...
data class MavlinkManualControl(
    val roll: Float,
    val pitch: Float,
    val throttle: Float,
    val yaw: Float,
...
object MavlinkInbound {
    ...
    /** MAVLink 2 header: magic, len, incompat, compat, seq, sysid, compid, msgid(3). */
    private const val SENDER_SYSTEM_OFFSET = 5
    ...
    /**
     * Which system sent a frame, or null when it is not a well-formed MAVLink 2 frame.
     *
     * A vehicle that broadcasts its telemetry from the socket it also listens on receives its own
     * frames back. Nothing in the payload says "mine", so this is what tells them apart from a
     * ground station speaking at the same time.
     */
    fun senderSystemOf(
        data: ByteArray,
        length: Int,
    ): Int? {
        if (validate(data, length) == null) return null
        return data[SENDER_SYSTEM_OFFSET].toInt() and 0xFF
    }
